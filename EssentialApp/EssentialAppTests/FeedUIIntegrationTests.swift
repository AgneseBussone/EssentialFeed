...
class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_loadFeedActions_requestFeedFromLoader() {
        ...
        XCTAssertEqual(loader.loadFeedCallCount, 1, "Expected another loading request once user initiates a load")
        ...
        sut.simulateUserInitiatedReload()
        XCTAssertEqual(loader.loadFeedCallCount, 3, "Expected a third  loading request once user initiates another refresh")
    }
    ...
    func test_loadMoreActions_requestMoreFromLoader() {
        ...
        XCTAssertEqual(loader.loadMoreCallCount, 0, "Expected no requests until load more is called")
        ...
        sut.simulateLoadMoreFeedAction()
        XCTAssertEqual(loader.loadMoreCallCount, 2, "Expected request after load more cempletes with more pages")

        loader.completeLoadMoreWithError(at: 1)
        sut.simulateLoadMoreFeedAction()
        XCTAssertEqual(loader.loadMoreCallCount, 3, "Expected request after load more fails")
    ...
    func test_loadingMoreIndicator_isVisibleWhileLoadingMore() {
        ...
        loader.completeFeedLoading(at: 0)
        XCTAssertFalse(sut.isShowingLoadMoreFeedIndicator, "Expected no loading indicator once loading is completed")

        sut.simulateLoadMoreFeedAction()
        XCTAssertTrue(sut.isShowingLoadMoreFeedIndicator, "Expected loading indicator on load more action")

        loader.completeLoadMore(at: 0)
        XCTAssertFalse(sut.isShowingLoadMoreFeedIndicator, "Expected no loading indicator once loading completes")
    ...
    func test_loadFeedCompletion_rendersSuccessfullyLoadedEmptyFeedAfterNonEmptyFeed() {
        ...
        sut.simulateLoadMoreFeedAction()
        loader.completeLoadMore(with: [image1], at: 0)
    ...
    func test_feedImageView_loadsImageURLWhenVisible() {
        ...
        XCTAssertEqual(loader.loadedImageURLs, [image0.url, image1.url], "Expected second image URL request once the view becomes visible")
    }
    ...
    func test_feedImageView_cancelsImageLoadingWhenNotVisibleAnymore() {
        ...
        XCTAssertEqual(loader.cancelledImageURLs, [image0.url], "Expected one cancelled image URL request once the image is not visible anymore")

        sut.simulateFeedImageViewNotVisible(at: 1)
        XCTAssertEqual(loader.cancelledImageURLs, [image0.url, image1.url], "Expected two cancelled image URL request once the second image is not visible anymore")
    }
    ...
    func test_feedImageViewLoadingIndicator_isVisibleWhileLoadingImage() {
        ...
        XCTAssertEqual(view0?.isShowingImageLoadingIndicator, false, "Expected no loading indicator for first view once the loading completes successfully")
        XCTAssertEqual(view1?.isShowingImageLoadingIndicator, true, "Expected loading indicator for second view once first image loading completes successfully")
        
        loader.completeImageLoadingWithError(at: 1)
        XCTAssertEqual(view0?.isShowingImageLoadingIndicator, false, "Expected no loading indicator for first view once second image loading completes with failure")
        XCTAssertEqual(view1?.isShowingImageLoadingIndicator, false, "Expected no loading indicator for second view once second image loading completes with failure")
    }
    ...
    func test_loadImageDataCompletion_dispatchesFromBackgroundThreadToMainThread() {
        ...
        let exp = expectation(description: "Waiting for bg thread")
        DispatchQueue.global().async {
