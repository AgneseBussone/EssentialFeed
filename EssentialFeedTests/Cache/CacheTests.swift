...
class CacheTests: XCTestCase {
    ...
    func test_save_requestNewCacheInsertWithTimestampOnSuccessfulDeletion() {
        ...
        let items = [uniqueItem(), uniqueItem()]
        let cacheItems = items.map { CacheItem(id: $0.id, description: $0.description, location: $0.location, imageURL: $0.imageURL)}

        sut.save(items) { _ in }
        store.completeDeletionSuccessfully()
        
        XCTAssertEqual(store.receivedMessages, [.deleteCacheFeed, .insert(cacheItems, timestamp)])
    }
